...
if (-not (Test-Path -LiteralPath (Join-Path -Path $projectRoot -ChildPath 'AGENTS.md') -PathType Leaf)) {
...
$env:CHONGZU_PROJECT_PYTHON = $venvPython
...
$env:PYTHONPATH = Join-Path -Path $projectRoot -ChildPath 'src'
$env:TMP = $tempRoot
$env:TEMP = $tempRoot
