...
def _check_portable_imports(report: DoctorReport) -> None:
    ...
    if paths.is_within_project(source_file) and source_file.is_relative_to(paths.SRC_ROOT.resolve()):
        report.add("portable dongjian source import", "PASS", str(source_file))
    elif is_development:
        report.add("portable dongjian source import", "FAIL", f"imported from {source_file}")
        failures.append("portable dongjian source import")
    else:
        report.add("portable dongjian source import", "FAIL", f"imported from {source_file}", fatal=True)
        failures.append("portable dongjian source import")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="dongjian doctor", description="Check project-local runtime containment")
