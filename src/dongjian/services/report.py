...
You compose one DongJian analysis report from selected, persisted Analysis Run
...
def render_report_html(record: Mapping[str, object]) -> str:
    ...
    title = esc(report.get("title") or record.get("title") or "DongJian report")
    body: list[str] = [
        f"<h1>{title}</h1>",
        ...
        *,
        ...
        try:
            ...
            if callable(cancellable) and cancel_event is not None:
                ...
                worker = Thread(target=invoke, name="dongjian-report-provider", daemon=True)
                try:
                    worker.start()
