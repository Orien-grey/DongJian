...
from dongjian.registry import Registry, canonical_source_root, utc_now as registry_now
from dongjian.scan import ScanError, scan_source
...
def _decode_text(path: Path) -> tuple[str, str]:
    ...
    for encoding in ("utf-8", "gb18030"):
        try:
            ...
            continue
    raise UnicodeDecodeError("dongjian", raw, 0, min(1, len(raw)), "unsupported text encoding")
...
def extract_text(
    ...
    *,
    ...
    try:
        ...
        exhausted = False
        executor = ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="dongjian-text")
        try:
            while pending or not exhausted:
                ...
                while not exhausted and len(pending) < worker_count * 2:
                    ...
                    try:
                        ...
                        exhausted = True
                        break
                    emit(text_source.relative_path, "读取文本")
                    ...
                    if reusable is not None and _artifacts_exist(reusable, workspace):
                        ...
                        completed += 1
                        emit(text_source.relative_path, "复用已有文本结果")
                        continue
                ...
                for future in done:
                    ...
                    completed += 1
                    emit(text_source.relative_path, "文本提取完成")
