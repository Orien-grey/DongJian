...
def _start_server_locked(project_root: Path, *, port: int = DEFAULT_PORT, open_browser: bool = True) -> int:
    if not (project_root / "frontend" / "dist" / "index.html").is_file():
        print("DongJian server cannot start: frontend/dist/index.html is missing; run the frontend build", file=sys.stderr)
        return 2
    ...
    if existing:
        ...
        if _healthy(
            ...
            print(f"DongJian server already running at http://{DEFAULT_HOST}:{active_port}/")
            if open_browser:
    ...
    try:
        ...
        print("DongJian 项目已有运行中的服务实例，当前服务未启动。", file=sys.stderr)
        return 3
        ...
        if existing and _state_scope_matches(existing, project_root, active_port):
            ...
            if previous_pid > 0 and process_identity(previous_pid) is None:
                if not _cleanup_orphaned_descendants(root_pid=previous_pid, project_root=project_root):
                    print("DongJian server 上一次异常退出后仍有无法确认归属的子进程，已取消启动", file=sys.stderr)
                    return 2
        state_path.unlink(missing_ok=True)
    if _port_in_use(port):
        print(f"DongJian server cannot start: {DEFAULT_HOST}:{port} is already in use", file=sys.stderr)
        return 2
    ...
    if identity is None:
        ...
        print("DongJian server 无法确认启动进程身份，已停止启动。", file=sys.stderr)
        return 2
    _write_state(
        state_path,
        {
    ...
    while time.monotonic() < deadline:
        if _healthy(port, expected_pid=process.pid, expected_instance=instance_id):
            print(f"DongJian server started at http://{DEFAULT_HOST}:{port}/ (PID {process.pid})")
            if open_browser:
                webbrowser.open(f"http://{DEFAULT_HOST}:{port}/")
            return 0
        if process.poll() is not None:
            state_path.unlink(missing_ok=True)
            print("DongJian server exited before health check; see workspace/logs/server.log", file=sys.stderr)
            return 2
        time.sleep(0.2)
    _terminate_started_process(process.pid)
    state_path.unlink(missing_ok=True)
    if _port_in_use(port):
        print(f"DongJian server cannot start: {DEFAULT_HOST}:{port} became occupied during startup", file=sys.stderr)
    else:
        print("DongJian server did not become healthy; see workspace/logs/server.log", file=sys.stderr)
    return 2
...
def start_server(project_root: Path, *, port: int = DEFAULT_PORT, open_browser: bool = True) -> int:
    ...
    try:
        ...
        print("DongJian server start is already in progress; please try again shortly", file=sys.stderr)
        return 3
    try:
...
def stop_server(project_root: Path, *, reset: bool = False) -> int:
    ...
    if not state:
        try:
            ...
            print("DongJian server 正在运行，但缺少可验证的 PID state，未执行终止。", file=sys.stderr)
            return 2
        probe.release()
        state_path.unlink(missing_ok=True)
        print("DongJian server is not running")
        return 0
    try:
        pid = int(state["pid"])
        ...
        if not _project_lock_available(project_root):
            print("DongJian server state 无法验证，且项目仍被占用；未执行终止。", file=sys.stderr)
            return 2
        state_path.unlink(missing_ok=True)
        print("已清理无法验证的 DongJian server state")
        return 0
    ...
    if not verified_identity:
        ...
        if process_identity(pid) is not None or not _project_lock_available(project_root):
            print("DongJian server 身份无法确认，未执行终止。", file=sys.stderr)
            return 2
        if not _state_scope_matches(state, project_root, port):
            ...
            # cannot be attributed to this DongJian instance safely.
            state_path.unlink(missing_ok=True)
            print("已清理不属于当前 DongJian 实例的 stale state")
            return 0
        ...
        if not _cleanup_orphaned_descendants(root_pid=pid, project_root=project_root):
            print("DongJian server 已退出，但仍有无法清理的项目子进程；保留 PID state。", file=sys.stderr)
            return 2
        state_path.unlink(missing_ok=True)
        print("已清理未运行的 DongJian server state")
        return 0
        ...
        try:
            request = Request(
                ...
                headers={
                    ...
                    "X-DongJian-Control": control_token,
                    "X-DongJian-Shutdown-Reason": "reset" if reset else "stop",
                },
            )
            ...
            if wait_for_owned_tree_exit(pid, known_descendants, GRACEFUL_STOP_TIMEOUT_SECONDS):
                state_path.unlink(missing_ok=True)
                print(f"Stopped DongJian server PID {pid}")
                return 0
        except (OSError, URLError, ValueError):
            pass
    if not verified_identity:
        print("DongJian server 未在 graceful timeout 内退出，且无法验证进程归属；未强制终止。", file=sys.stderr)
        return 2
    if sys.platform == "win32":
        ...
        if current_job is None:
            ...
            if root_still_owned:
                ...
                if not _wait_for_verified_root_exit(state, project_root, FORCED_STOP_VERIFY_TIMEOUT_SECONDS):
                    print("DongJian server 无法停止；已验证的 PID 未被终止。", file=sys.stderr)
                    return 2
        if not wait_for_owned_tree_exit(pid, known_descendants, FORCED_STOP_VERIFY_TIMEOUT_SECONDS):
            print("DongJian server 强制终止后仍未退出，保留 PID state 供再次诊断。", file=sys.stderr)
            return 2
        state_path.unlink(missing_ok=True)
        print(f"Stopped DongJian server PID {pid}")
        return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="dongjian server lifecycle")
