...
def _logger(project_root: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("dongjian.api")
...
class DongJianHTTPServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def _handler_type(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "DongJianLocal/1"
            ...
            def _dispatch(self, body: bytes = b"") -> None:
                ...
                if parsed.path.startswith("/api/"):
                    if parsed.path == "/api/v1/internal/shutdown" and self.command == "POST":
                        if not app_server_token_matches(self.server, self.headers.get("X-DongJian-Control")):
                            self._send_error(ApiError("not_found", "resource was not found", 404))
                            return
                        app.request_shutdown(reset=self.headers.get("X-DongJian-Shutdown-Reason") == "reset")
                        self._send_json(202, {"status": "shutting_down"})
                        threading.Thread(target=self.server.shutdown, name="dongjian-shutdown", daemon=True).start()
                        return
                    try:
...
def app_server_token_matches(server: DongJianHTTPServer, token: str | None) -> bool:
...
def run_server(
    *,
    ...
    try:
        ...
        print("DongJian server 启动失败，请查看 workspace/logs/server.log。", file=sys.stderr)
        finish_ownership(False)
        return 2
    try:
        server = DongJianHTTPServer((host, port), app, control_token=control_token)
    except OSError as exc:
        logger.error("could not bind %s:%s: %s", host, port, exc, exc_info=True)
        finish_ownership(True)
        print(f"DongJian server could not bind {host}:{port}: {exc}", file=sys.stderr)
        return 2
    print(f"DongJian local server listening at http://{host}:{port}/", flush=True)
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="dongjian local server")
