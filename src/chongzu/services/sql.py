...
class SqlQueryService:
    ...
    def _run_query(frames: list[tuple[str, pl.DataFrame]], sql: str) -> tuple[list[str], list[dict[str, Any]], bool, float]:
        ...
        def worker() -> None:
            ...
            try:
                ...
                for alias, frame in frames:
                    ...
                    # not a ChongZu dependency.  The replacement scan uses
                ...
                cursor = connection.execute(
                    f"SELECT * FROM ({sql}) AS __chongzu_result LIMIT {SQL_MAX_RESULT_ROWS + 1}"
                )
                columns = [item[0] for item in cursor.description]
        ...
        thread = threading.Thread(target=worker, name="chongzu-safe-sql", daemon=True)
        thread.start()
